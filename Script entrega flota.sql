...
INSERT INTO flota.coches(matricula,grupo,marca,modelo,fecha_compra,color,aseguradora,n_poliza,fecha_alta_seguro,importe_revision,moneda,kms_revision,fecha_revision,kms_totales) VALUES
...
matricula VARCHAR(50) primary key, Id_modelo INT not null, Id_color INT not null,numero_km int not null, fecha_entrega date not null);	

create table flota.grupo_asegurador(Id_grupo_asegurador SERIAL primary key, nombre_grupo_asegurador VARCHAR(50) not NULL);

create table flota.seguros (numero_poliza int primary key, matricula varchar (50) , id_grupo_asegurador int not null, fecha_alta_seguro date not null);

create table flota.colores (id_color SERIAL primary key, color varchar(50) not null);

create table flota.revision (id_revision SERIAL primary key, matricula varchar(50) not null, fecha_revision date not null, km_revision int not null, importe_revision float not null, id_moneda int not null);

create table flota.modelo (id_modelo SERIAL primary key, nombre_modelo varchar (50)not null, id_marca int not null);

create table flota.moneda (id_moneda SERIAL primary key, nombre_moneda varchar (50) not null);

create table flota.marca (id_marca SERIAL primary key, nombre_marca varchar (50) not null, id_fabricante int not null);

create table flota.grupo_fabricante (id_fabricante SERIAL primary key, fabricante varchar(50) not null 	);

alter table flota.seguros add constraint pk_grupo_asegurador foreign key (id_grupo_asegurador) references flota.grupo_asegurador;

alter table flota.seguros add constraint pk_matricula foreign key (matricula) references flota.vehiculo;

alter table flota.vehiculo add constraint pk_color foreign key (Id_color) references flota.colores;

alter table flota.revision add constraint pk_matricula foreign key (matricula) references flota.vehiculo;

alter table flota.revision add constraint pk_moneda foreign key (Id_moneda) references flota.moneda;

alter table flota.vehiculo add constraint pk_modelo foreign key (Id_modelo) references flota.modelo;

alter table flota.marca add constraint pk_marca foreign key (Id_fabricante) references flota.Grupo_fabricante;



insert into flota.grupo_asegurador (nombre_grupo_asegurador) 
select aseguradora from flota.coches c group by aseguradora order by aseguradora;

insert into flota.colores (color) 
select color from flota.coches c group by color order by color;

insert into flota.grupo_fabricante (fabricante)
select grupo from flota.coches c group by grupo order by grupo;


insert into flota.moneda (nombre_moneda)
select moneda from flota.coches c group by moneda order by moneda;

insert into flota.marca (nombre_marca, id_fabricante)
select marca, id_fabricante  from flota.coches join flota.grupo_fabricante gf on grupo = fabricante group by id_fabricante, marca ;

insert into flota.modelo (nombre_modelo, id_marca)
select modelo , id_marca  from flota.coches join flota.marca m  on marca=nombre_marca  group by modelo, id_marca ;

insert into flota.vehiculo (matricula, id_modelo, id_color, numero_km, fecha_entrega) 
select matricula, id_modelo, id_color, kms_totales, fecha_compra from flota.coches c join flota.modelo m on modelo=nombre_modelo join flota.colores c2 on c.color=c2.color
group by matricula, id_modelo, id_color, kms_totales, fecha_compra;

insert into flota.seguros ( numero_poliza, matricula, id_grupo_asegurador, fecha_alta_seguro)
select  n_poliza, matricula, id_grupo_asegurador, fecha_alta_seguro  from flota.coches c join flota.grupo_asegurador ga on aseguradora=nombre_grupo_asegurador
group by matricula, n_poliza, id_grupo_asegurador, fecha_alta_seguro;

insert into flota.revision(matricula, fecha_revision, km_revision, importe_revision, id_moneda) 
select matricula, fecha_revision, kms_revision, importe_revision, id_moneda from flota.coches c join flota.moneda m on moneda=nombre_moneda 
group by matricula, fecha_revision, kms_revision, importe_revision, id_moneda;

select v.matricula, nombre_modelo, nombre_marca, fabricante, fecha_entrega, color, numero_km, nombre_grupo_asegurador, numero_poliza 
from flota.vehiculo v  join flota.modelo m  on v.id_modelo = m.id_modelo join flota.marca m2  on m.id_marca = m2.id_marca join flota.grupo_fabricante gf on m2.id_fabricante=gf.id_fabricante join flota.colores c  on v.id_color=c.id_color  join flota.seguros s on v.matricula=s.matricula join flota.grupo_asegurador ga on s.id_grupo_asegurador=ga.id_grupo_asegurador; 
